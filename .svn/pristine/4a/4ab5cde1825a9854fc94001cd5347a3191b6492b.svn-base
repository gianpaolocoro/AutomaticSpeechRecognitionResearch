...
public class ExhaustiveDecoderParallel {
	...
	int overallInitializations = 0;
	double[][] features;

	
	public synchronized void addF(String word, fmodel f) {
		wordperF.put(word, f);
	}
		...
		public Integer call() throws Exception {
			try {
				...
				if (m != null) {
					...
					double[][] feats = Arrays.copyOf(features, features.length);
					
					fmodel fm = new fmodel(m, feats);
					//wordperF.put(word, fm);
					addF(word,fm);
				}
			} catch (Exception e) {
				System.out.println("Error executing thread " + word);
				e.printStackTrace();
			}
			updateIdx();
			return null;
	...
	public String decode(double[][] features) {
		...
		for (String word : allLMWords) {
			...
			if (m != null) {
				...
				DThread thread = new DThread(word);
		...
		long time = 0;
		while (overallInitializations < counter) {
			try {
				Thread.sleep(1000);
			...
			time += 1000;
			if (time % 10000 == 0) {
				System.out.println("Initialized " + overallInitializations + " of " + counter);
			}
		}
		
		System.out.println("All transcriptions finished");
		executorService.shutdown();
		overallInitializations=0;
		
		//end of parallel initialization
		
		allWords = new String[allAMWords.size()];
		allWords = allAMWords.toArray(allWords);
		
		for (String word : wordperF.keySet()) {
			System.out.println("Maximising V for "+word);
